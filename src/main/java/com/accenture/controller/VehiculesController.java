...
@ApiResponse(responseCode = "400", description = "Données invalides")
    ...
    @GetMapping("/search/date/categories")
    ...
    @ApiResponse(responseCode = "201", description = "Véhicules trouver avec succès")
    @ApiResponse(responseCode = "400", description = "Données invalides")
    ResponseEntity<VehiculeDTO> rechercherParDatesEtCategories(LocalDate dateDebut, LocalDate dateFin, boolean inclureMotos, boolean inclureVoitures){
        VehiculeDTO parDatesEtCategorie = vehiculeService.rechercherParDateEtType(dateDebut, dateFin, inclureMotos, inclureVoitures);
        log.info("Rechercher par dates et catégories de véhicules : {} ", parDatesEtCategorie);
        return ResponseEntity.ok(parDatesEtCategorie);
    }
